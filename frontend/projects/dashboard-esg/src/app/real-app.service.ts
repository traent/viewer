...
import { AppService, formatAddress } from './app.service';
...
export class RealAppService extends AppService {
  ...
  readonly productPresentation$ = this.documentsService.getDocumentByTag(DOC_PRODUCT_PRESENTATION_TAG);

  readonly campaign$ = this.documentsService.getDocumentByTag(DOC_CAMPAIGN_TAG);
  ...
  readonly communications$ = this.documentsService.getDocumentsByTag(DOC_COMMUNICATION_TAG);
  ...
  async getAddress(): Promise<string> {
    return firstValueFrom(this.organization$.pipe(
      map((organization) => organization && organization.address
  ...
  async getWeb(): Promise<string | undefined> {
    return firstValueFrom(this.organization$.pipe(
      map((organization) => organization && organization.website
